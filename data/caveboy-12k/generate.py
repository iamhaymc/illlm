...
def parse_tool_result(content: Any) -> Optional[str]:
    ...
    if isinstance(parsed, dict) and "content" in parsed and {"name", "tool_call_id"} & parsed.keys():
        inner = parsed["content"]
         # `default=str` survives an object `ast.literal_eval` recovered that JSON
         # cannot serialise (a `...` literal -> Ellipsis), so one odd row degrades
         # to its string form instead of aborting the whole build.
        return inner if isinstance(inner, str) else json.dumps(inner, ensure_ascii=False, default=str)
    return text
