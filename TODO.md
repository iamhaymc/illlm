...
3. **A perplexity command.** There is no number for what `--quant q8` costs in
   accuracy; it is reported as a correlation against `transformers`, which says
   the two agree and not what either is worth. Everything below that trades
   accuracy for speed — q4, a narrower key/value cache, a sparse activation —
   is unlandable without one number that moves in a known direction, so this is
   cheap infrastructure that several later items depend on.

4. **q4 weights.** Halving the bytes a token reads is the largest remaining
   ...
   dispatch all widen rather than fork. Land it behind item 3's number.

5. **Fuse the attention score row.** Scores are materialised per head before
...
6. **Pack the weight panel for prefill.** `dense` streams weight rows in their
...
7. **An AMX path for the q8 dot where the host has one.** The VNNI path folds
...
8. **Store the key/value cache at half width.** It is f32 today, which is the
...
9. **Runtime SIMD dispatch.** The vector width is chosen at compile time, so a
...
13. **Streaming decode of partial UTF-8.** A token whose bytes end mid-sequence
...
14. **Per-channel rather than per-block q8 scales**, as an alternative layout to
...
15. **Write a packed engine file**, so a q8 or q4 repack is done once rather
...
16. **Rotary scaling types beyond `default` and `linear`** — `yarn`, `llama3`,
...
17. **Sliding window attention**, if a member of the family uses it. The layer
    plan already carries a per-layer kind, so it is a third case rather than a
    change of shape.

18. **The mixture-of-experts variant** (`model_type: "lfm2_moe"`). A router and
...
19. **Batched sequences: several states advanced in one forward pass**, which
    ...
    Item 1's mark and rewind is the harder half of it, and item 7 wants it.

20. **Speed the added-token scan.** It is linear in the number of added tokens
...
21. **Evaluate the Jinja `chat_template`** for the subset chat templates
...
22. **Replace the character-class range table with generated Unicode property
...
23. **Unigram and WordPiece tokenizer models, and the Metaspace pre-tokenizer.**
    Reported as `ILL_VOCAB` rather than approximated, which is the right
    refusal and still a refusal.

24. **Implement a device backend against the existing `IllBackend` seam.** The
...
25. **Run the full harness against the published `LiquidAI/LFM2.5-2.6B`
...
26. **Build and run the test suite on arm64.** The NEON instantiation of the
...
27. **Run the caveman tune against the published weights.** The pipeline is
...
28. **Run the abliteration against the published weights.** The drive is
...
29. **Grow `data/tune.jsonl` past its 202 hand written rows** with `--make-data`
...
30. **Draft with the model's own q4 weights, verify with its q8 weights.**
...
31. **A shortlist for the vocabulary head.** *Hypothesis*, *model-preserving if
...
32. **Training-free activation sparsity.** *Adaptation* (TEAL), *approximate*.
    ...
    **Risk**: the win needs a gather, and a gather over a memory bound stream
    ...
    the sparsity where item 3's perplexity rises by less than 0.1.

33. **Lookup-table mixed-precision matrix multiply.** *Adaptation* (T-MAC),
    ...
    **Experiment**: only after item 4 exists, against its dot product.
    **Stop rule**: abandon if it does not beat the direct q4 dot by 1.2x on
    decode.

34. **Keys quantised per channel, values per token.** *Established* (KIVI),
    ...
    **Experiment**: after item 8, extend it downward. **Stop rule**: abandon
    below 8 bits if perplexity rises by more than 0.05.

35. **Skip attention layers to make a self-draft.** *Hypothesis*, approximate
    ...
    the same as item 30 — below 1.6 tokens at k=4, it does not pay.
...
36. **The vision variant** (`model_type: "lfm2_vl"`). A patch embedding and an
    image tower ahead of the same stack, and a second token stream to splice
    into the prompt.

37. **An audio front end**, if a member of the family grows one. The same shape
