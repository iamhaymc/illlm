...
written in pure optimized C and without dependencies.

It runs a Hugging Face checkpoint directly —
`config.json`, the `*.safetensors` shards, and `tokenizer.json` — 
with nothing to convert and nothing to install.
...
./build/app_main info      --model ckpt/0.4b
./build/app_main generate --model ckpt/2.6b --prompt "Write a haiku about rivers."
...
checkpoints ship in `ckpt/` — `ckpt/0.4b` (LFM2.5-350M), `ckpt/1.2b`
(LFM2.5-1.2B-Instruct), and `ckpt/2.6b` (LFM2.5-2.6B) — so the checkpoint
suite — logits, tokenizer, throughput, and greedy behaviour against the
reference — runs by default against `ckpt/0.4b`; `--no-checkpoint` skips it,
...
`build/tune/merged` in the same layout as `ckpt/0.4b`.
...
python3 util/tune.py --model ckpt/0.4b --uncensor     # abliterate into build/tune/uncensored
python3 util/tune.py --model ckpt/0.4b --train        # LoRA adapter into build/tune
python3 util/tune.py --model ckpt/0.4b --merge        # fold it into build/tune/merged
...
weights to `build/tune/uncensored` in the same layout as `ckpt/0.4b`, so the engine
...
python3 util/eval.py --model ckpt/2.6b            # the nine benchmarks
python3 util/eval.py --model ckpt/2.6b --samples 200   # a longer run
python3 util/eval.py --tasks ifeval,boolq --model ckpt/2.6b
python3 util/eval.py --model ckpt/2.6b --rival hf-inference-providers/Qwen/Qwen3-4B
...
plus `setup`, `close`, and `width`. 

The CPU backend fills that table with threaded kernels. 
A device backend fills the same table and keeps its queue in `inner`. 
