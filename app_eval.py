#!/usr/bin/env python3
"""app_eval.py -- scores a checkpoint on the public benchmarks, and draws the result.
...
itself still has no dependencies; this is a workflow helper like `app_tune.py`,
...
def engine_build(skip):
    """Builds app_main through run.py unless it is already there."""
    if os.path.exists(ENGINE.binary):
        return True
    if skip:
    ...
    code = subprocess.call([sys.executable, os.path.join(ROOT, "run.py"), "build"])
...
def table_write(path, book):
    ...
    note = (f"\n{cap}, {book['made'][:10]}. "
            "Long Horizon Execution is generated by `app_eval.py`, not the "
...
def chart_foot(book):
    ...
    where = " · ".join(parts)
    made = "Long Horizon Execution is generated by app_eval.py, not the paper's rows"
...
def main(argv=None):
    ...
    if args.model and not engine_build(args.no_build):
        raise SystemExit("the engine did not build; run python3 run.py build")
